...
- **Noted but not changed (content, not code, needs your decision) — asked the owner 2026-06-25, both deferred:** the LINE links across the whole site (navbar, footer, hero, LINE floating button) all point to `https://line.me/ti/p/@tirehub` — the pre-rebrand handle. Owner wasn't sure if this is still the real LINE OA account and will check with whoever manages it — **don't re-flag this automatically next session, just ask once if it's come up.** Also, the footer phone link is `tel:+6620000000`, which looks like a placeholder — owner said leave it as-is for now, not a priority.
...
Size-based search already queries the real DB live (`TyreFinder.tsx` → `hooks/useProducts.ts` → `/api/products?width=&profile=&rimSize=`, indexed Prisma columns, no migration needed — and as of 2026-06-25 this is actually wired into the shop page, see Bugs fixed above). The old dead mock hook `hooks/useFilters.ts`'s `useMatchingTyres()` was removed (2026-06-23) — it had zero call sites. What's still genuinely mock: vehicle/plate lookup (`hooks/useTyreFinder.ts` + `lib/tyreFinder/fitmentData.ts`) — needs a real vehicle-fitment dataset or external API (e.g. TecDoc, DLT-based plate lookup) to become fully real. Medium-large effort, mostly external-data-source integration.
- **2026-06-25: mock data expanded and a real format bug fixed.** Found that `platePlaceholder` for Thai showed `เช่น กข 1234` (correct Thai plate format) but the mock lookup table only had UK-format keys like `AB12CDE` — a Thai user typing exactly what the UI told them to could never get a match, and the English locale's placeholder (`e.g. AB12 CDE`) baked in the same UK assumption for a Thailand-only business. Fixed: `MOCK_PLATE_LOOKUP` now uses real Thai plate format keys (e.g. `กข1234`), and both locales' placeholders show the same Thai-format example. Also rebalanced `VEHICLE_FITMENTS` from 8 mostly-European entries (BMW/Audi/Ford) to 28 entries weighted toward the actual Thai market — Toyota/Honda/Isuzu/Mazda/Mitsubishi/Nissan/Ford/Suzuki sedans, SUVs, and 1-ton pickups (Hilux, D-Max, Fortuner, MU-X, Triton, Ranger — a huge, previously-unrepresented segment here), with a few premium imports kept for variety. Expanded `PROFILES_BY_WIDTH`/`RIMS_BY_PROFILE` (Tab 1 size cascade) to cover the high-profile pickup/SUV sizes (60/65/70 profiles, 17"/18" rims on 245/255/265 widths) so these fitments are also browsable manually, not just reachable via the vehicle tab. Still mock data, not a live external API — just much more representative of who's actually using this site.
